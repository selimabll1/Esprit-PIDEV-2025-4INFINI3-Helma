...
export class SessionService {
  ...
  saveSession(
    ...
    const normalized = this.normalizeSessionArgs(
      tokenOrPayload, roleOrRememberMe, email, rememberMeOrUserId, userId
    );
  ...
  setSession(
    ...
    this.saveSession(tokenOrPayload, roleOrRememberMe, email, rememberMeOrUserId, userId);
  }
  ...
  getPortalRoute(): string {
    switch (this.getRole()) {
      case 'YOUTH_BENEFICIARY': return '/youth';
      case 'INVESTOR':          return '/investor';
      case 'ADMIN':
      case 'COMPLIANCE':        return '/admin/dashboard';
      default:                  return '/';
    }
  }

  clearSession(): void {
    for (const key of [TOKEN_KEY, ROLE_KEY, EMAIL_KEY, USER_ID_KEY, LEGACY_AUTH_KEY]) {
  ...
  private normalizeSessionArgs(
    ...
    if (typeof tokenOrPayload === 'string') {
      ...
      const rememberMe =
        typeof roleOrRememberMe === 'boolean' ? roleOrRememberMe :
        typeof rememberMeOrUserId === 'boolean' ? rememberMeOrUserId : true;
      const resolvedUserId =
        typeof rememberMeOrUserId === 'number' ? rememberMeOrUserId :
  ...
  private getLegacyAuth(): any | null {
    const raw = localStorage.getItem(LEGACY_AUTH_KEY) ?? sessionStorage.getItem(LEGACY_AUTH_KEY);
    if (!raw) return null;
    try { return JSON.parse(raw); } catch { return null; }
  }

  private getLegacyToken(): string | null {
    const l = this.getLegacyAuth();
    return l?.token ?? l?.accessToken ?? l?.jwt ?? l?.data?.token ?? l?.data?.accessToken ?? null;
  }

  private getLegacyRole(): string | null {
    const l = this.getLegacyAuth();
    return l?.role ?? l?.user?.role ?? l?.userRole ?? l?.data?.role ?? l?.data?.user?.role ?? null;
  }

  private getLegacyEmail(): string | null {
    const l = this.getLegacyAuth();
    return l?.email ?? l?.user?.email ?? l?.data?.email ?? l?.data?.user?.email ?? null;
  }

  private getLegacyUserId(): string | null {
    const l = this.getLegacyAuth();
    const v = l?.userId ?? l?.id ?? l?.user?.id ?? l?.data?.userId ?? l?.data?.id ?? l?.data?.user?.id ?? null;
